...
from src.auth.eligibility import email_allowed
from src.auth.hash_functions import get_secret_hash
from src.common import app_logger
from src.common.utils import idam_decode, idam_encode
from src.common.constants import PERF_TEST
from src.models.access_tokens import AccessModel
from src.schemas.access_schema import AccessSchema
...
class Authenticator:
    ...
    def __init__(
        ...
        if not client:
            ...
            local_dev = os.getenv("LOCAL_DEV", None)
            if not local_dev:
    ...
    def sign_up(self, user):
        ...
        user_attributes = [
            ...
            {"Name": "custom:organisation", "Value": user.organisation},
            {"Name": "custom:terms", "Value": user.terms},
        ]

        if self.user_pool["Name"].startswith("cf-challenge-authoring"):
            # Then default is Challenge Author
            # Check eligibility
            if not email_allowed(user.email):
                return {"message": "User not eligible."}, requests.codes.unauthorized
            ...
            user.user_type == UserType.COMPETITOR or user.user_type == UserType.GUARDIAN
        ):
            user_attributes.append(
                {"Name": "custom:organisation_role", "Value": user.organisation_role},
            )
            user_attributes.append(
                {"Name": "custom:address_line_1", "Value": user.address_line_1},
            )
            user_attributes.append(
                {"Name": "custom:address_line_2", "Value": user.address_line_2},
            )
            user_attributes.append(
                {"Name": "custom:city", "Value": user.city},
            )
            user_attributes.append(
                {"Name": "custom:county", "Value": user.county},
            )
            user_attributes.append(
                {"Name": "custom:country", "Value": user.country},
            )
            user_attributes.append(
                {"Name": "custom:how_found", "Value": user.how_found},
            )
            user_attributes.append(
                {"Name": "custom:before", "Value": user.before},
            )
            user_attributes.append(
                {"Name": "custom:permission", "Value": user.permission},
            )
            user_attributes.append(
                {"Name": "custom:postcode", "Value": user.postcode},
            )
            user_attributes.append(
                {
                    "Name": "custom:last_seen",
                    "Value": datetime.utcnow().replace(tzinfo=simple_utc()).isoformat(),
                },
            )
            user_attributes.append(
                {"Name": "custom:mfa_setup_complete", "Value": "0"},
            )

            user_attributes.append(
                {"Name": "custom:verified_at", "Value": ""},
            )
        else:
            return {"message": "Unknown user type."}, requests.codes.bad_request

        client_id = self.user_pool["IdamClient"]["ClientId"]
        client_secret = self.user_pool["IdamClient"]["ClientSecret"]

        logger.debug(self.user_pool["IdamClient"])

        if user.terms != "1":
            return {
                "message": "Terms must be agreed to and set to 1."
        ...
        try:
            response = self.cognito_client.sign_up(
                ...
                SecretHash=get_secret_hash(
                    user.username, client_id=client_id, client_secret=client_secret
                ),
            ...
            if (
                user.user_type == UserType.AUTHOR
                or user.user_type == UserType.GUARDIAN
                or user.user_type == UserType.COMPETITOR
            ):
                # Add user to a group
                _, http_status = self.add_user_to_group(
                    username=response["UserSub"], group_name=user.user_type
                )
    ...
    def login(
        ...
        """
        # pool_id = self.user_pool["Id"]
        # callback_urls = self.user_pool["CallbackUrls"]

        # if redirect_uri not in callback_urls:
        #     logger.error("Invalid redirect uri.")
        #     return {"message": "Invalid redirect uri."}, requests.codes.forbidden

        if account_type == "local":
            # client_id = os.getenv("LOCAL_AUTHORITY_CLIENT_ID")
            # userpool_id = os.getenv("LOCAL_USERPOOL_ID")
            client_id = "6bbco0950b3hf2qk424krsfkp8"
            userpool_id = " eu-west-2_mIbzJx0HO"

        else: 
            # client_id = os.getenv("AGENT_CLIENT_ID")
            # userpool_id = os.getenv("AGENT_USERPOOL_ID")
            client_id = "31ee4gh0dm5kvdjqckssgo4vm8"
            userpool_id = "eu-west-2_nA0dDYh3U"
            ...
            access_record = access_schema.load(
                {
                    ...
                    "refresh_token": res["AuthenticationResult"][
                        "RefreshToken"
                    ],
                    "session_id": "",
                }
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Force change password::::: under_18:{under_18}")
                ...
                return {
                    ...
                    "pool_name": self.user_pool["Name"],
                    "client_id": self.user_pool["IdamClientNoSecret"]["ClientId"],
                }, requests.codes.ok

            if under_18 == True or PERF_TEST:  # or user is for performance testing
                challenge_name = res.get("ChallengeName", "IDAM_NO_MFA")
                if PERF_TEST:
                    logger.warn("Performance testing enabled!")
                    challenge_name = "IDAM_NO_MFA"
            else:
                challenge_name = res.get("ChallengeName", "MFA_RESET")
                try:
                    # get user details
                    access_token = res["AuthenticationResult"]["AccessToken"]
                except Exception as e:
            ...
            logger.debug("Challenge Name")
            logger.debug(challenge_name)

            # setup mfa
            if challenge_name == "MFA_RESET":
                logger.debug("MFA_SETUP:true")
                r = self.cognito_client.associate_software_token(
                ...
                return {
                    ...
                    "secret_code": r["SecretCode"],
                }, r["ResponseMetadata"]["HTTPStatusCode"]

            if challenge_name == "MFA_SETUP":
                logger.debug("MFA_SETUP:true")
                r = self.cognito_client.associate_software_token(Session=res["Session"])
                self.update_last_seen(username)
                return {
                    "username": username,
                    "challenge_name": "MFA_SETUP",
                ...
                logger.debug("SOFTWARE_TOKEN_MFA:true")
            ...
            elif challenge_name == "IDAM_NO_MFA":
                ...
                try:
                    ...
                    session["client_id"] = self.user_pool["IdamClientNoSecret"][
                        "ClientId"
                    ]
                ...
                logger.debug("Unknown challenge_name")
